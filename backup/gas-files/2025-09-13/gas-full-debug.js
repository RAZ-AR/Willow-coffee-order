...
function t_(k, lang) {
  var d = {
    greet: {
      en: '🌿 Welcome to Willow Coffee! ☕\n\nYour personal loyalty card:',
      ru: '🌿 Добро пожаловать в Willow Coffee! ☕\n\nВаша персональная карта лояльности:',
      sr: '🌿 Dobrodošli u Willow Coffee! ☕\n\nVaša lična kartica lojalnosti:'
    },
    youCard: { en: '💳 Card', ru: '💳 Карта', sr: '💳 Kartica' },
    orderReceived: {
      en: '✅ Thank you for your order!\n\n🍳 We are starting to prepare your delicious coffee.',
      ru: '✅ Спасибо за ваш заказ!\n\n🍳 Начинаем готовить ваш вкусный кофе.',
      sr: '✅ Hvala na vašoj porudžbini!\n\n🍳 Počinjemo sa pripremom vaše ukusne kafe.'
    },
    ...
    readyNotify: {
      en: 'We will notify you when your order is ready!',
      ru: 'Мы уведомим вас, когда ваш заказ будет готов!',
      sr: 'Obavestićemo vas kada vaša porudžbina bude spremna!'
    },
    thankYou: {
      en: 'Thank you for choosing Willow Coffee!',
      ru: 'Спасибо, что выбрали Willow Coffee!',
      sr: 'Hvala vam što ste izabrali Willow Coffee!'
    }
  };
...
function apiOrder_(payload) {
  ...
  var groupHtml =
    '🔔 <b>' + t_('newOrder', 'en') + ' #' + card + '</b>\n\n' +
    '👤 <b>Customer:</b> ' + nick + '\n' +
    '💳 <b>' + t_('youCard', 'en') + ':</b> <code>' + card + '</code>\n' +
    '⏱️ <b>' + t_('when', 'en') + ':</b> ' + whenHtml + '\n' +
    '💰 <b>' + t_('payment', 'en') + ':</b> ' + payment + '\n\n' +
    '📦 <b>' + t_('items', 'en') + ':</b>\n' + itemsHtml + '\n\n' +
    '💵 <b>' + t_('sum', 'en') + ':</b> ' + total + ' RSD\n\n' +
    '🚀 <i>Let\'s prepare this order!</i>';

  var langU = langFromUser_(baseU);
  var clientHtml =
    t_('orderReceived', langU) + '\n\n' +
    '👤 <b>' + nick + '</b>\n' +
    '💳 <b>' + t_('youCard', langU) + ':</b> <code>' + card + '</code>\n' +
    '⏱️ <b>' + t_('when', langU) + ':</b> ' + whenHtml + '\n' +
    '💰 <b>' + t_('payment', langU) + ':</b> ' + payment + '\n\n' +
    '📦 <b>' + t_('items', langU) + ':</b>\n' + itemsHtml + '\n\n' +
    '💵 <b>' + t_('sum', langU) + ':</b> ' + total + ' RSD\n\n' +
    '⏰ <i>' + t_('readyNotify', langU) + '</i>\n' +
    '🙏 <i>' + t_('thankYou', langU) + '</i>';
...
function handleStart_(update) {
  ...
  var html =
    '<b>👋 Hi, ' + nick + '!</b>\n\n' +
    t_('greet', lang) + '\n\n' +
    '<b>' + t_('youCard', lang) + ':</b> <code>' + card + '</code>\n' +
    '⭐ <b>Stars:</b> ' + stars + '\n\n' +
    '🚀 <i>Ready to order? Open our mini-app!</i>';
  tgSendHTML_(u.id, html);
}

function adjustStarsFromMessage_(text, chatId) {
  console.log("🔍 PROD DEBUG: adjustStarsFromMessage_ processing:", text, "from chat:", chatId);
  var m = String(text || '').match(/^\s*(\d{4})\s*([+\-])\s*(\d+)\s*$/);
  if (!m) {
    console.log("🔍 PROD DEBUG: No match for star command format");
    return { ok:false, reason:'no match - card must be 4-digit' };
  }
  ensureHeaders_();
...
function doPost(e) {
  ...
  if (data && data.message) {
    ...
    var allowChat = String(chatId) === String(_prop('CASHIER_GROUP_ID','')) ||
                    ...
                    String(chatId) === String(_prop('STARS_CHANNEL_ID',''));
    if (!allowChat) {
